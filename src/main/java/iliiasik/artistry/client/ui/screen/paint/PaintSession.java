...
public class PaintSession {
    ...
    public void open() {
        ...
        if (connected()) {
            PacketDistributor.sendToServer(new CanvasViewC2SPacket(targetEntity.getBlockPos(), true));
            viewRegistered = true;
        }
    }
    ...
    public void close() {
        if (targetEntity == null || !viewRegistered) return;
        if (connected()) {
            PacketDistributor.sendToServer(new CanvasViewC2SPacket(targetEntity.getBlockPos(), false));
        }
    ...
    public void lockImage(UUID uuid, boolean lock) {
        PacketDistributor.sendToServer(new LockCanvasImageC2SPacket(target(), uuid, lock));
    }

    public void deleteImage(UUID uuid) {
        PacketDistributor.sendToServer(new DeleteCanvasImageC2SPacket(target(), uuid));
    }

    public void togglePixelize(UUID uuid) {
        PacketDistributor.sendToServer(new TogglePixelizeC2SPacket(target(), uuid));
    }

    public void uploadImage(byte[] bytes) {
        PacketDistributor.sendToServer(new UploadImageC2SPacket(target(), bytes));
    }
    ...
    private void sendImageMove(UUID uuid) {
        ...
        PacketDistributor.sendToServer(new MoveCanvasImageC2SPacket(
    ...
    private void flushPixels() {
        ...
        if (connected()) {
            PacketDistributor.sendToServer(new SaveCanvasC2SPacket(target(), changes));
        }
    }
    ...
    public void maybeSendCursor(short gx, short gy) {
        ...
        if (connected()) {
            PacketDistributor.sendToServer(new CanvasCursorC2SPacket(targetEntity.getBlockPos(), gx, gy));
        }
    }
