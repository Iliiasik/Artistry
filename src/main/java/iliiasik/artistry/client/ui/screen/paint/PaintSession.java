...
public class PaintSession {
    ...
    public CanvasData canvasData() { return canvasData; }
    public CanvasImageLayer imageLayer() { return imageLayer; }
    public CanvasPresence presence() { return presence; }
    public boolean isWorld() { return targetEntity != null; }
    ...
    public void open() {
        ...
        if (connected()) {
            ModNetwork.sendToServer(new CanvasViewC2SPacket(targetEntity.getBlockPos(), true));
            viewRegistered = true;
        }
    }
    ...
    public void close() {
        if (targetEntity == null || !viewRegistered) return;
        if (connected()) {
            ModNetwork.sendToServer(new CanvasViewC2SPacket(targetEntity.getBlockPos(), false));
        }
    ...
    public void lockImage(UUID uuid, boolean lock) {
        ModNetwork.sendToServer(new LockCanvasImageC2SPacket(target(), uuid, lock));
    }

    public void deleteImage(UUID uuid) {
        ModNetwork.sendToServer(new DeleteCanvasImageC2SPacket(target(), uuid));
    }

    public void togglePixelize(UUID uuid) {
        ModNetwork.sendToServer(new TogglePixelizeC2SPacket(target(), uuid));
    }
    ...
    private void sendImageMove(UUID uuid) {
        ...
        ModNetwork.sendToServer(new MoveCanvasImageC2SPacket(
    ...
    private void flushPixels() {
        ...
        if (connected()) {
            ModNetwork.sendToServer(new SaveCanvasC2SPacket(target(), changes));
        }
    }
    ...
    public void maybeSendCursor(short gx, short gy) {
        ...
        if (connected()) {
            ModNetwork.sendToServer(new CanvasCursorC2SPacket(targetEntity.getBlockPos(), gx, gy));
        }
    }
