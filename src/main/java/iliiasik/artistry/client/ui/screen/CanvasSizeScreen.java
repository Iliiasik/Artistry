    ...
    private void onSizeChosen(int size) {
        if (targetEntity != null) {
            ...
            if (Minecraft.getInstance().getConnection() != null) {
                PacketDistributor.sendToServer(new SetCanvasSizeC2SPacket(new PosterTarget.World(targetEntity.getBlockPos()), size));
            }
            Minecraft.getInstance().setScreen(new PaintScreen(targetEntity));
        } else if (targetStack != null) {
            if (Minecraft.getInstance().getConnection() != null) {
                PacketDistributor.sendToServer(new SetCanvasSizeC2SPacket(new PosterTarget.Held(targetHand), size));
            }
