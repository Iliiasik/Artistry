...
public class CanvasData {
    ...
    public static boolean isSizeChosenForStack(net.minecraft.world.item.ItemStack stack) {
        var comp = stack.get(net.minecraft.core.component.DataComponents.CUSTOM_DATA);
    ...
    public void fromNbt(CompoundTag nbt) {
        revision++;
        canvasSize = Math.clamp(nbt.getInt("size"), 0, MAX_SIZE);
        if (nbt.contains("p")) {
    ...
    public void copyFrom(CanvasData other) {
        revision++;
        this.canvasSize = Math.clamp(other.canvasSize, 0, MAX_SIZE);
        for (int y = 0; y < MAX_SIZE; y++) {
