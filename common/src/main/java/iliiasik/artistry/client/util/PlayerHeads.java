...
public final class PlayerHeads {
    ...
    public static ResourceLocation skinOf(@Nullable UUID uuid) {
        ...
        return DefaultPlayerSkin.get(uuid == null ? Util.NIL_UUID : uuid).texture();
    }

    public static void draw(GuiGraphics ctx, ResourceLocation skin, int x, int y, int size) {
        ctx.blit(skin, x, y, size, size,
                FACE_U, FACE_V, FACE_SIZE, FACE_SIZE,
                SKIN_TEXTURE_SIZE, SKIN_TEXTURE_SIZE);
        ctx.blit(skin, x, y, size, size,
                HAT_U, HAT_V, FACE_SIZE, FACE_SIZE,
                SKIN_TEXTURE_SIZE, SKIN_TEXTURE_SIZE);
    }
}
